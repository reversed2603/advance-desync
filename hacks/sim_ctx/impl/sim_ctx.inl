...
namespace nem::hacks {
	__forceinline void c_sim_context::check_velocity ( extrapolation_data_t& data ) const {
		static auto sv_max_vel = valve___g_cvar->find_var ( xorstr_ ( "sv_maxvelocity" ) );
	...
	__forceinline void c_sim_context::trace_player_bbox ( extrapolation_data_t& data, vec3_t& start, vec3_t& end, valve::trace_t* trace ) const {
		...
		filter.m_ignore_entity = valve::g_local_player.operator nem::valve::c_player * ( );

		valve___g_engine_trace->trace_ray ( ray, valve::e_mask::contents_solid, reinterpret_cast < valve::trace_filter_t* > ( &filter ), trace );
	}

	__forceinline void c_sim_context::friction ( extrapolation_data_t& data ) const {
		static auto sv_friction = valve___g_cvar->find_var ( xorstr_ ( "sv_friction" ) );
		static auto sv_stopspeed = valve___g_cvar->find_var ( xorstr_ ( "sv_stopspeed" ) );
		...
		auto drop = control * friction * valve___g_global_vars->m_interval_per_tick;
	...
	__forceinline void c_sim_context::air_accelerate ( extrapolation_data_t& data, vec3_t& wish_dir, float wish_spd ) const {
		static auto sv_airaccelerate = valve___g_cvar->find_var ( xorstr_ ( "sv_airaccelerate" ) );
		...
		auto accel_spd = sv_airaccelerate->get_float ( ) * wish_niggas * valve___g_global_vars->m_interval_per_tick * data.m_player->surface_friction ( );
	...
	__forceinline void c_sim_context::try_player_move ( extrapolation_data_t& data ) const {
		valve::trace_t trace;
		vec3_t end_pos = data.m_origin + data.m_velocity * valve___g_global_vars->m_interval_per_tick;
	...
	__forceinline void c_sim_context::accelerate ( extrapolation_data_t& data, vec3_t& wish_dir, float wish_spd, float accel ) const {
		...
		auto accel_spd = accel * valve___g_global_vars->m_interval_per_tick * accelerate_scale * data.m_player->surface_friction ( );
	...
	__forceinline void c_sim_context::walk_move ( extrapolation_data_t& data ) const {
		...
		static auto sv_stepsize = valve___g_cvar->find_var ( xorstr_ ( "sv_stepsize" ) );
		valve::trace_t trace;
		static auto sv_accelerate = valve___g_cvar->find_var ( xorstr_ ( "sv_accelerate" ) );

		data.m_dir = math::to_deg( std::atan2f( data.m_velocity.y, data.m_velocity.x ) );

		vec3_t right {};
		vec3_t fwd {};
		vec3_t wish_vel {};
		math::angle_vectors ( qangle_t ( 0.f, data.m_dir, 0.f ), &fwd, &right );
		...
		static auto sv_maxspeed = valve___g_cvar->find_var ( xorstr_ ( "sv_maxspeed" ) );
		...
		dest [ 0 ] = data.m_origin [ 0 ] + data.m_velocity [ 0 ] * valve___g_global_vars->m_interval_per_tick;
		dest [ 1 ] = data.m_origin [ 1 ] + data.m_velocity [ 1 ] * valve___g_global_vars->m_interval_per_tick;
