...
namespace nem::hacks {
	...
	bool is_armored ( valve::c_player* player, int hit_group ) {
		const bool has_helmet = player->has_helmet ( );
		const bool has_heavy_armor = player->has_heavy_armor ( );
		const float armor_val = player->armor_value ( );
	...
	) const
	{
		...
		static auto mp_dmg_scale_ct_head = valve___g_cvar->find_var ( xorstr_ ( "mp_damage_scale_ct_head" ) );
		static auto mp_dmg_scale_t_head = valve___g_cvar->find_var ( xorstr_ ( "mp_damage_scale_t_head" ) );

		static auto mp_dmg_scale_ct_body = valve___g_cvar->find_var ( xorstr_ ( "mp_damage_scale_ct_body" ) );
		static auto mp_dmg_scale_t_body = valve___g_cvar->find_var ( xorstr_ ( "mp_damage_scale_t_body" ) );

		float head_dmg_scale = team == 3 ? mp_dmg_scale_ct_head->get_float ( ) : mp_dmg_scale_t_head->get_float ( );
		const float body_dmg_scale = team == 3 ? mp_dmg_scale_ct_body->get_float ( ) : mp_dmg_scale_t_body->get_float ( );

		const auto armored = is_armored ( player, hitgroup );
		const bool has_heavy_armor = player->has_heavy_armor ( );
		const bool is_zeus = valve::g_local_player->weapon ( ) ? valve::g_local_player->weapon ( )->item_index ( ) == valve::e_item_index::taser : false;

		const auto armor_val = static_cast < float > ( player->armor_value ( ) );
