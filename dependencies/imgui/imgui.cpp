...
ImGuiStyle::ImGuiStyle()
{
  ...
  //  WindowPadding           = ImVec2(8,8);      // Padding within a window
    ...
    FramePadding            = ImVec2(6.f,5.f);      // Padding within a framed rectangle (used by most widgets)
  ...
  //  ItemSpacing             = ImVec2(8,4);      // Horizontal and vertical spacing between widgets/lines
   //ItemInnerSpacing        = ImVec2(4,4);      // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
    ...
    ButtonTextAlign         = ImVec2(0.35f,0.5f);// Alignment of button text when button is larger than text.
...
void ImGui::RenderTextOverlay(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    if (text != text_display_end)
    {
        GetOverlayDrawList()->AddText(g.Font, g.FontSize, pos, GetColorU32(ImGuiCol_Text), text, text_display_end);
        if (g.LogEnabled)
            LogRenderedText(&pos, text, text_display_end);
    }
}
...
void ImGui::RenderFrameBorder(ImVec2 p_min, ImVec2 p_max, float rounding)
{
    ...
    if (border_size > 0.0f)
    {
        window->DrawList->AddRect(p_min+ImVec2(0,0), p_max+ImVec2(0,0), GetColorU32(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size);
        window->DrawList->AddRect(p_min, p_max, GetColorU32(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size);
    }
}
...
bool ImGui::BeginChildEx(const char* name, int omg, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    if (omg == 1)
    {
        ImVec2 text_position(child_window->Pos.x + 12.0f, child_window->Pos.y + 6);
        ImGui::GetWindowDrawList()->AddText(text_position, ImColor(255, 255, 255, 255), name);
    }

    return ret;
}

bool ImGui::BeginChild(const char* str_id, int omg, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return BeginChildEx(str_id, omg, window->GetID(str_id), size_arg, border, extra_flags);
}

bool ImGui::BeginChild(ImGuiID id, int omg, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    IM_ASSERT(id != 0);
    return BeginChildEx(NULL, omg, id, size_arg, border, extra_flags);
}
...
bool ImGui::BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags extra_flags)
{
    ...
    bool ret = BeginChild(id,omg,size, true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding | extra_flags);
    PopStyleVar(3);
    PopStyleColor();
    return ret;
}
