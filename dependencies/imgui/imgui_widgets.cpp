...
#endif
#include "../../../csgo/imgui_internal.h"

#include <ctype.h>      // toupper
#if defined(_MSC_VER) && _MSC_VER <= 1520 // MSVC 2008 or earlier
...
#ifdef _MSC_VER
#pragma warning (disable: 4147)     // condition expression is constant
#pragma warning (disable: 4996)     // 'This function or variable may be unsafe': strcpy, strdup, sprintf, vsnprintf, sscanf, fopen
#if defined(_MSC_VER) && _MSC_VER >= 1922 // MSVC 2019 16.2 or later
#pragma warning (disable: 5254)     // operator '|': deprecated between enumerations of different types
#endif
#endif
...
static const signed char    IM_S8_MIN  = -148;
static const signed char    IM_S8_MAX  = 147;
...
bool ImGui::TabButtonEx(const char* label, int number, bool* active, const ImVec2& size_arg, bool some_wiki_wiki_stuff )
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos - ImVec2 ( 0, -23 );
    ...
    const ImVec4 hover_act = ImColor(71, 71, 76, 255);
    ...
    //ImGui::PushFont ( nem::hacks::g_visuals->m_fonts.m_mojesh_ubrat_ee_ya_razreshayu );
    if ( some_wiki_wiki_stuff )
    GetWindowDrawList ( )->AddText ( bb.Min - ImVec2 ( -3, 59 ), GetColorU32 ( ImVec4 ( 1.f, 1.f, 1.f, 1.f ) ), "A" );

    //ImGui::PopFont ( );

   //ImGui::PushFont ( nem::hacks::g_visuals->m_fonts.m_menu_icons_fontik );

    if ( number == 1 )
        GetWindowDrawList ( )->AddText ( bb.Min, GetColorU32 ( ImVec4 ( it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second ) ), "h" );
    if (number == 2)
        GetWindowDrawList ( )->AddText ( bb.Min, GetColorU32 ( ImVec4 ( it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second ) ), "a" );
    if (number == 3)
        GetWindowDrawList ( )->AddText ( bb.Min, GetColorU32 ( ImVec4 ( it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second ) ), "g" );
    if (number == 4)
        GetWindowDrawList ( )->AddText ( bb.Min, GetColorU32 ( ImVec4 ( it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second ) ), "c" );
    if (number == 5)
        GetWindowDrawList ( )->AddText ( bb.Min, GetColorU32 ( ImVec4 ( it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second ) ), "d" );
...
bool ImGui::TabButton(const char* label, int number, bool active, const ImVec2& size_arg, bool wiki_wiki )
{
    return TabButtonEx(label, number, &active, size_arg, wiki_wiki);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImRect bb ( pos, pos + size - ImVec2 ( 0, 5 ) );
    ItemSize ( size, style.FramePadding.y );
    if ( !ItemAdd ( bb, id ) )
        return false;
...
bool ImGui::SubTabEx(const char* label,  bool* active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (hovered && *active && it_filled->second >= 0.5f)
        it_filled->second = clamp(it_filled->second - 2.2f * ImGui::GetIO().DeltaTime, 0.5f, 1.f);
    else
        it_filled->second = clamp(it_filled->second + (2.2f * ImGui::GetIO().DeltaTime * ((*active) ? 1.f : -1.f)), 0.0f, 1.f);


    PushStyleColor(ImGuiCol_Text, ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second));
    RenderText(ImVec2(bb.Min.x, bb.Min.y), label);
    PopStyleColor();
...
bool ImGui::TabEx(const char* label, int number, bool* active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = ImVec2(48, 14);
    const ImVec2 second_label_size = ImVec2(252, 6);
    ...
    if (g.LastActiveId == g.CurrentWindow->GetID(label))
   {
    ...
    const ImVec4 hover_act = ImColor(255, 255, 255, 255);
    const ImVec4 hover_dis = ImColor(70, 70, 75, 255);
   ...
   // if (number == 1)
    //    GetWindowDrawList()->AddImage(oneicon, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(ImVec4(0, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)));
  //  if (number == 2)
   //     GetWindowDrawList()->AddImage(twoicon, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)));
//if (number == 3)
   //     GetWindowDrawList()->AddImage(threeicon, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)));
  //  if (number == 4)
   //     GetWindowDrawList()->AddImage(fouricon, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)));
   // if (number == 5)
   //     GetWindowDrawList()->AddImage(fiveicon, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)));
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    if (hovered)
        window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f), col, 14);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    ImVec2 center = bb.GetCenter();
    if (hovered || held)
        window->DrawList->AddCircleFilled(center/*+ ImVec2(0.0f, -0.5f)*/, g.FontSize * 0.5f + 1.0f, bg_col, 14);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(ImVec2(window->DC.CursorPos.x + 5.f, window->DC.CursorPos.y + 5.f ), window->DC.CursorPos - ImVec2( 5.f, 5.f ) + ImVec2(label_size.y + style.FramePadding.y * 2, label_size.y + style.FramePadding.y * 2));
    ...
    const ImVec4 hover_dis = ImColor(255, 255, 255, 255);
    const ImVec4 hover_act = ImColor(19, 19, 23, 255);
    ...
    RenderFrame(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)), !*v, 2.2f);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Separator), 3.f);
            if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos - ImVec2(8, 0) + ImVec2(16, 0), ImVec2(35, 0) + window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    ...
    const ImVec4 hover_act = ImColor( 18, 18, 19, 255);
    const ImVec4 hover_dis = ImColor( 18, 18, 19 );
    ...
    if (it_arrow_alpha->second.open != popup_open)
    {
        if (!it_arrow_alpha->second.reverse)
        {
            ...
            it_arrow_alpha->second.alpha = clamp(it_arrow_alpha->second.alpha - (8.4f * ImGui::GetIO().DeltaTime), 0.01f, 1.f);
        }
        else
        {
            ...
            it_arrow_alpha->second.alpha = clamp(it_arrow_alpha->second.alpha + (2.4f * ImGui::GetIO().DeltaTime), 0.01f, 1.f);
        }
    }
    else
    {
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
       RenderFrame(frame_bb.Min, ImVec2(value_x2 + 20, frame_bb.Max.y), frame_col, 1.f, style.FrameRounding );
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
       ...
       // RenderFrame(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, GetColorU32(ImGuiCol_TitleBg), 1.f, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
        //GetWindowDrawList()->AddImage(arrowz, ImVec2(value_x2 - 8, frame_bb.Min.y + 3), ImVec2(value_x2 + 16, frame_bb.Max.y - 2), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_hover->second));
        RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y - 5, frame_bb.Min.y + style.FramePadding.y ), GetColorU32(ImVec4(50 / 255.f, 50 / 255.f, 50 / 255.f, 1.f)), it_arrow_alpha->second.dir, 1.0f);
    }

    //window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_hover->second), 1.f);

    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(3,0), ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
    if ( label_size.x > 0 )
    {
        RenderText ( ImVec2 ( frame_bb.Min.x + 2, frame_bb.Min.y - 20 - style.FramePadding.y + 4 ), label );
    }

    it_open->second = clamp(it_open->second + (3.f * ImGui::GetIO().DeltaTime * (popup_open ? 1.f : -1.f)), 0.01f, 1.f);
    ...
    PushStyleVar(ImGuiStyleVar_Alpha, it_open->second * 1.f);
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w +27, 0.0f), ImVec2(FLT_MAX, ((CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)) * it_open->second)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    if (window->DC.CurrLineTextBaseOffset != window->DC.PrevLineTextBaseOffset)
        SetCursorPosY(GetCursorPosY() + 1.f * 7.f);
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() - 1.f * 3.f;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(10, 13 * 1.f) + ImVec2(8,4) - ImVec2(0,12), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f * 1.f) - ImVec2(0, 9) + ImVec2(43, 0));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max - ImVec2(0, 13) + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0));

    ItemSize(ImRect(total_bb.Min - ImVec2(0, 22 * 1.f), total_bb.Max), style.FramePadding.y);
    if (!ItemAdd(ImRect(total_bb.Min - ImVec2(0, 22 * 1.f), total_bb.Max), id, &frame_bb))
        return false;
    ...
    // Draw frame
    const ImVec4 hover_act2 = ImColor(255, 255, 255, 255);
    const ImVec4 hover_dis = ImColor(255, 255, 255, 255);
    ...
    RenderFrame(frame_bb.Min + ImVec2(3, 0) - ImVec2(0,1), frame_bb.Max - ImVec2(5, 4) + ImVec2(4,3), ImColor( 26, 27, 29, 255), false, g.Style.FrameRounding);
   // window->DrawList->AddRect(frame_bb.Min + ImVec2(4, 0), frame_bb.Max - ImVec2(5, 0), ImGui::GetColorU32(it_hover->second), g.Style.FrameRounding);

    // Slider behavior
    ImRect grab_bb;
    ImRect slider_bb = ImRect(frame_bb.Min - ImVec2(13,4), frame_bb.Max + ImVec2(13,0));
   ...
   //     window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x , grab_bb.Min.y - 3), ImVec2(grab_bb.Max.x, grab_bb.Max.y + 3), GetColorU32(it_grab->second), 3.f);
      // window->DrawList->AddImage(grabz, ImVec2(grab_bb.Min.x - 18, grab_bb.Min.y - 24), ImVec2(grab_bb.Max.x + 18, grab_bb.Max.y + 24), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(hover_act32));
    RenderFrame(frame_bb.Min + ImVec2(3, 0) - ImVec2(0, 0), grab_bb.Max - ImVec2(13,0), ImGui::GetColorU32(it_hover->second), false, g.Style.FrameRounding);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    value_buf_end + char(".0");
    PushStyleColor(ImGuiCol_Text, ImVec4(1.f,1.f,1.f, 1.f));
    RenderTextClipped(ImVec2(frame_bb.Max.x - 1 - CalcTextSize(value_buf).x, frame_bb.Min.y + 7  - CalcTextSize(value_buf).y - (12 * 1.f)), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.0f, 0.0f));
    
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y + 7 - CalcTextSize(label).y - (12 * 1.f)), label);
    PopStyleColor();
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags);
    return value_changed;
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    bool value_changed = InputTextEx(label, buf, buf_size, bb.GetSize(), flags);
    if (init)
    {
        // First frame we started displaying the InputText widget, we expect it to take the active id.
        IM_ASSERT(g.ActiveId == id);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx(label, buf_size, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            // Store text height (note that we haven't calculated text width at all, see GitHub issues #383, #1404)
            if (is_multiline)
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect_size.x <= 0.0f) rect_size.x = IM_FLOOR(g.Font->GetCharAdvance((ImWchar)' ') * 0.52f); // So we can see selected empty lines
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const float w_inputs = w_full - w_button - 95.f;
    ...
    const ImVec2 pos = ImVec2(window->DC.CursorStartPos.x + 170, window->DC.CursorPos.y + 2);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2 ( 22, 17 ) ) )
        {
    ...
    if ((window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect) && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropTarget())
    {
        ...
        {
            memcpy((float*)col, payload->Data, sizeof(float) * 3); // Preserve alpha if any //-V514
...
bool ImGui::ColorEdit5(const char* label, bool value, float col[4], ImGuiColorEditFlags flags)
{
    if (value == true)
    {
        ImGui::SameLine();
        ...
        const ImVec2 pos = ImVec2(window->DC.CursorStartPos.x + 175, window->DC.CursorPos.y - 2);
        const float inputs_offset_x = (style.ColorButtonPosition == ImGuiDir_Left) ? w_button : 0.0f;
        window->DC.CursorPos.x = pos.x + inputs_offset_x;
        ...
        if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
        {
            ...
            const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
            if (ColorButton("##ColorButton", col_v4, flags, ImVec2(24, 24)))
            {
                if (!(flags & ImGuiColorEditFlags_NoPicker))
                {
                    // Store current color and open a picker
...
void ImGui::RenderColorRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max, ImU32 col, float grid_step, ImVec2 grid_off, float rounding, int rounding_corners_flags)
{
    ...
    if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF)
    {
        ...
        ImU32 col_bg2 = GetColorU32(ImAlphaBlendColor(IM_COL32(148,148,148,255), col));
...
bool ImGui::ColorPicker5(const char* label, bool value, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    if (value == true)
    {
        ImGuiContext& g = *GImGui;
        ...
        if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
        {
            ...
            if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
                if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
                {
                    ...
                    // For the later we don't want to run the hue-wrap canceling code. If you are well versed in HSV picker please provide your input! (See #2052)
        ...
        const ImU32 col_midgrey = IM_COL32(148, 148, 148, style_alpha8);
        ...
        if (flags & ImGuiColorEditFlags_PickerHueWheel)
        {
            ...
            const int segment_per_arc = ImMax(4, (int)wheel_r_outer / 14);
            for (int n = 0; n < 6; n++)
            {
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const float spacing_L = IM_FLOOR(spacing_x * 0.52f);
    const float spacing_U = IM_FLOOR(spacing_y * 0.52f);
    bb.Min.x -= spacing_L;
    bb.Min.y -= spacing_U;
    ...
    const ImVec4 text_dis = ImColor(225, 225, 225, 255);
    const ImVec4 text_act = ImColor(255, 255, 255, 255);

    const ImVec4 hover_act = ImColor( 34, 35, 39, 255);
    const ImVec4 hover_dis = ImColor( 34, 35, 39, 255);
    ...
    if (hovered && selected && it_filled->second >= 1.5f)
        it_filled->second = clamp(it_filled->second - 2.2f * ImGui::GetIO().DeltaTime, 0.5f, 1.f);
    else
        it_filled->second = clamp(it_filled->second + (2.2f * ImGui::GetIO().DeltaTime * ((selected) ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    if (hovered || selected)
    {
        const ImU32 col = GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second));
    ...
    ImU32 col_bg = ImColor(31, 31, 39, 255);
    ImU32 col_bg2 = ImColor(31, 31, 39, 255);
    ImU32 col_bg3 = ImColor(31, 31, 39, 255);
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL && !(window->Flags & ImGuiWindowFlags_MenuBar))
        {
            ...
            //GetForegroundDrawList()->AddTriangleFilled(ta, tb, tc, moving_within_opened_triangle ? IM_COL32(0,148,0,148) : IM_COL32(148,0,0,148)); // [DEBUG]
        }
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
...
#if 0
    if (hovered && g.HoveredIdNotActiveTimer > 0.52f && bb.GetWidth() < tab->ContentWidth)
    {
    ...
    if (g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > 0.52f && IsItemHovered())
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding, y1 + rounding), rounding, 9, 14);
    ...
    if (g.Style.TabBorderSize > 0.0f)
    {
        ...
        draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding - 0.5f, y1 + rounding + 0.5f), rounding, 9, 14);
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    PushID(0x25223347 + (str_id ? 0 : columns_count));
