...
namespace nem::valve {
	...
	__forceinline void material_var_t::set_comp_vec ( const float val, const int comp ) {
	...
	__forceinline std::uint32_t user_cmd_t::calc_checksum( ) {
		...
		return reinterpret_cast< fn_t >( g_context->addresses( ).m_user_cmd_calc_checksum.get( ) )( this );
	}

	__forceinline vec3_t& c_collideable::obb_mins ( )
	{
		using fn_t = vec3_t& ( __thiscall* )( decltype( this ) );

		return ( *reinterpret_cast< fn_t** >( this ) )[ 1u ] ( this );
	}

	__forceinline vec3_t& c_collideable::obb_maxs ( )
	{
		using fn_t = vec3_t& ( __thiscall* )( decltype( this ) );
	...
	__forceinline e_solid_type c_collideable::solid_type ( )
	{
		using fn_t = e_solid_type ( __thiscall* )( decltype( this ) );
	...
	inline studio_hitbox_t* studio_hdr_t::p_hitbox ( int i, int set )
	{
		auto s = hitbox_set ( set );
		if ( !s )
			return NULL;
	...
	__forceinline void bf_write_t::write_user_cmd( user_cmd_t* const to, user_cmd_t* const from ) {
		...
		reinterpret_cast< fn_t >( g_context->addresses( ).m_write_user_cmd.get( ) )( this, to, from );
	...
	__forceinline void anim_state_t::reset( ) {
		reinterpret_cast< void( __thiscall* )( decltype( this ) ) >(
			g_context->addresses( ).m_reset_anim_state.get( )
		)( this );
	}
	...
	__forceinline void anim_state_t::update(
		...
		reinterpret_cast< void( __vectorcall* )( decltype( this ), void*, float, float, float, void* ) >( 
			g_context->addresses( ).m_update_anim_state.get( )
		)( this, nullptr, roll, yaw, pitch, nullptr );
	}
}