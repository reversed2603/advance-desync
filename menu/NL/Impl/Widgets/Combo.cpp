...
C_Element& C_Element::Combo( uint32_t u32Hash, const char* szText, ImVec2 sPos, std::vector < std::string > cVecStrings, int* pValue )
{
    m_eType = ET_COMBO;
    ...
    m_cMapVars[_h( "CreateCallback" )] = C_Var( _h( "ComboCreateCallback" ) );
...
void C_Nl::ComboCreateCallback( C_Element* pElement )
{
    ...
    *pValue = std::clamp( *pValue, 0, ( int ) ( pVec->size( ) - 1 ) );
    ...
    if ( iOpenState )
    {
        ...
        ImGui::Begin( std::string( "##iss_" ).append( cText ).c_str( ), 0, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoSavedSettings );
    }
    ...
    ImGui::BeginChild( std::string( "##test_chld1___" ).append( cText ).c_str( ), 0, ImVec2( 74.f + flAddSizeX, 27.f + flAddSizeY ), true, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoBringToFrontOnFocus );
    ...
    ImGui::BeginChild( std::string( "##test_chld2___" ).append( cText ).c_str( ), 0, ImVec2( 72.f + flAddSizeX, 25.f + flAddSizeY ), true, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoBringToFrontOnFocus );
    ImGui::EndChild( );
    ...
    ImGui::BeginChild( std::string( "##test_chld3___" ).append( cText ).c_str( ), 0, ImVec2( 70.f + flAddSizeX, 23.f + flAddSizeY ), true, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoBringToFrontOnFocus );

    ImGui::PushStyleColor( ImGuiCol_Text, pElement->GetVar( _h( "TextColor" ) ).GetVec4( ) );
    ImGui::Text( pVec->empty( ) ? "None" : pVec->at( *pValue ).c_str( ) );
    ImGui::PopStyleColor( );
    ...
    for ( int i = 0; i < pVec->size( ); ++i )
    {
        ...
        ImGui::PushStyleColor( ImGuiCol_Text, *pValue == i ? RGB_VEC( 255.f, 255.f, 255.f, 255.f ) : AnimateTextColor( pElemsProps->at( i ).m_flColorScale, pElemsProps->at( i ).m_flTextAlpha ) );
    ...
    g_NL->ComboMouseIntersectCallback( pElement, bIntersected );

    bool bIntersectedFull = g_NL->CheckMouseIntersectionLocal( sPos, ImVec2( ImGui::GetWindowSize( ).x - 20.f, 30.f ) );

    if ( u32Hash && bIntersectedFull && ImGui::GetIO( ).MouseClicked[1] )
        g_Keybinds->EnableKeybindSelection( pElement, ET_COMBO, u32Hash );

    g_NL->ComboFullMouseIntersectCallback( pElement, bIntersectedFull );
    
    if ( !pVec->empty ( ) && ImGui::GetIO ( ).MouseClicked [ 0 ] )
            g_NL->ComboClickCallback ( pElement, bIntersected );
}

void C_Nl::ComboMouseIntersectCallback( C_Element* pElement, bool bIntersected )
{
...
void C_Nl::ComboFullMouseIntersectCallback( C_Element* pElement, bool bIntersected )
{
...
void C_Nl::ComboClickCallback( C_Element* pElement, bool bIntersected )
{
    int& iOpenState = pElement->GetVar( _h( "OpenState" ) ).GetInt( );
    std::vector < C_BindComboItemProperty >* pElemsProps = pElement->GetVar( _h( "ElementsProps" ) ).GetRef< std::vector < C_BindComboItemProperty > >( );
